...
BEGIN
  IF EXISTS (
    ...
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Overlapping appointments for the same doctor are not allowed.';
  END IF;
END;
//
...
DELIMITER ;
    ...
    Patient.Patient_ID,
    Patient.Name,
    COUNT(Appointment.Appointment_ID),
...
SELECT 
    ...
    COUNT(Appointment.Appointment_ID) AS Upcoming_Appointment_Count
FROM 
    Doctor
JOIN 
    Appointment ON Doctor.Doctor_ID = Appointment.Doctor_ID
WHERE 
    Appointment.Date >= '2025-05-05'
    AND Appointment.Date < '2025-06-25'
GROUP BY 
